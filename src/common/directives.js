...
.factory('ribbon', ['$rootScope', function ($rootScope) {
    var ribbon = {
        ...
        updateBreadcrumb: function (crumbs) {
            crumbs.push("<i class='fa-fw fa fa-home txt-color-orangeDark'></i>Home");
...
angular.module('snowWhite.localize', [])
...
.factory('localize', ['$http', '$rootScope', '$window', function ($http, $rootScope, $window) {
    var localize = {
        ...
        getLangUrl: function (lang) {
            return 'assets/langs/' + lang.langCode + '.json';
        },
...
angular.module('snowWhite.navigation', [])
...
.directive('navGroup', function () {
    return {
        ...
        template: '<li data-ng-class="{active: active}"><a href=""><i data-ng-if="hasIcon" class="{{ icon }}"><em data-ng-if="hasIconCaption"> {{ iconCaption }} </em></i><span class="menu-item-parent" data-localize="{{ title }}">{{ title }}</span></a><ul data-ng-transclude=""></ul></li>'
...
.directive('navItem', ['ribbon', '$window', function (ribbon, $window) {
    return {
        ...
        transclude: true,
        replace: true,
        template: '<li data-ng-class="{active: isActive(view)}"><a href="{{ getItemUrl(view) }}" target="{{ getItemTarget() }}" title="{{ title }}"><i data-ng-if="hasIcon" class="{{ icon }}"><em data-ng-if="hasIconCaption"> {{ iconCaption }} </em></i><span ng-class="{\'menu-item-parent\': !isChild}" data-localize="{{ title }}"> {{ title }} </span><span data-ng-transclude=""></span></a></li>'
    };
}])
;

// directives for activity
angular.module('snowWhite.activity', [])
...
.directive('activityButton', function () {
    return {
        ...
        template: '<span id="activity" class="activity-dropdown"><i ng-class="icon"></i><b class="badge"> {{ total }} </b></span>',
...
.directive('activityItem', function () {
    return {
        ...
        transclude: true,
        replace: true,
        template: '<label class="btn btn-default" data-ng-click="loadItem()" ng-class="{active: active}"><input type="radio" name="activity"><span data-ng-transclude=""></span></label>',
        link: function (scope, element, attrs, parentCtrls) {
...
angular.module('snowWhite.smartui', [])
...
.directive('widgetHeader', function () {
    return {
        ...
        template: '<header><span class="widget-icon"> <i data-ng-class="icon"></i> </span><h2>{{ title }}</h2><span data-ng-transclude></span></header>'
    };
})
