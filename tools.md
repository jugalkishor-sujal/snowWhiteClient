# The Tools Used in `ngSnowWhite`

## Introduction

`ngSnowWhite` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `ngSnowWhite` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`ngSnowWhite` uses git to manage its codebase. While in theory you don't have
to use Git once you download `ngSnowWhite`, this project makes the assumption
...
While `ngSnowWhite` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `ngSnowWhite` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `ngSnowWhite`; Grunt is
...
to get started with `ngSnowWhite`. But for those curious or looking to go a
...
Grunt is the engine behind `ngSnowWhite`. It's the magic that makes it move.
...
`ngSnowWhite` comes with a `bower.json` file that looks something like this:
...
in `ngSnowWhite` by default in there to see what I mean.
